...
-- orders
insert into sales.orders(order_id, order_date, ORDER_REQUIRED_DATE, ORDER_SHIPPED_DATE, ORDER_FREIGHT, ORDER_SHIP_ADDRESS, ORDER_SHIP_CITY)
select * from dblink('localhost',
        'select order_id, order_date, required_date, shipped_date, freight, ship address, ship city')
as data(order_id integer, order_date TIMESTAMP, ORDER_REQUIRED_DATE TIMESTAMP, ORDER_SHIPPED_DATE TIMESTAMP, ORDER_FREIGHT MONEY, ORDER_SHIP_ADDRESS varchar(60), ORDER_SHIP_CITY varchar(15))
...
-- products
insert into sales.products(PROD_ID, PROD_NAME, PROD_QUANTITY, PROD_PRICE, PROD_IN_STOCK, PROD_ON_ORDER, PROD_REORDER_LEVEL, PROD_DISCONTINUED)
select * from dblink('localhost',
        'select order_id, order_date, required_date, shipped_date, freight, ship address, ship city')
as data(ORDET_ORDER_ID integer, ORDET_PROD_ID integer, ORDET_PRICE MONEY, ORDET_QUANTITY SMALLINT, ORDET_DISCOUNT REAL)
