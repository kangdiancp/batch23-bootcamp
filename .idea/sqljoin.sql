...
alter table departments  add constraint Amanager_fk foreign key (manager_id) references employees (employee_id)
...
CREATE TABLE job_history (
	...
	job_id varchar (10),
	department_id integer,
	CONSTRAINT EMP_DATE_PK PRIMARY KEY (employee_id, start_date), 
	FOREIGN KEY (job_id) REFERENCES jobs (job_id),
	FOREIGN KEY (department_id) REFERENCES departments (department_id),
...
insert into employees (employee_id, first_name, last_name, email, phone_number, hire_date, job_id, salary, commision_pct, manager_id, department_id, xemp_id) values
